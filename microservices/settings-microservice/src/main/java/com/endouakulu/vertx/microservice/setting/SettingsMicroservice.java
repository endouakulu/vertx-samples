...
public class SettingsMicroservice extends AbstractVerticle {
    ...
    private void createSetting(JDBCClient client, RoutingContext routingContext) {
        final SettingRequest setting = Json.decodeValue(routingContext.getBodyAsString(),
                                                        SettingRequest.class);
    ...
    private void deleteSetting(JDBCClient client, RoutingContext routingContext) {
        client.getConnection(conn -> {
            if (conn.failed()) {
    ...
    private void findBySettingId(JDBCClient client, RoutingContext routingContext) {
        client.getConnection(conn -> {
            if (conn.failed()) {
                System.err.println(conn.cause().getMessage());
                return;
    ...
    private void findAll(JDBCClient client, RoutingContext routingContext) {
        client.getConnection(conn -> {
            if (conn.failed()) {
                logger.error(conn.cause().getMessage());
            }
